...
# Этап 7.1: маркер N) (со скобкой) тоже распознаётся — модель копирует стиль
# источника, где списки заданы как «1) ... 2) ...». После разбиения скобка
# нормализуется к точке для единообразия (вариант А).
...
def _split_inline_numbers(line: str) -> str:
    ...
    Этап 7.1: нормализует маркер N) → N. для единообразия (вариант А).
    """
    m = _NUM_INLINE_RE.search(line)
    if not m:
        return line
    head = line[: m.start() + 1]
    rest = line[m.end():]  # начинается с «1. item 2. item ...» или «1) item 2) item ...»
    # Нормализуем первый маркер (он без предшествующего пробела).
    rest = re.sub(r"^(\d+)[.)][ \t]+", r"\1. ", rest)
    # Остальные « N. » / « N) » → «\nN. » (скобка нормализуется к точке).
    rest = re.sub(r"[ \t]+(\d+)[.)][ \t]+", r"\n\1. ", rest)
    return head + "\n" + rest
