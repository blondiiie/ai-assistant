...
- [x] Тестовый корпус Q&A (20 пар: 10 relevant + 10 out-of-scope), `data/qa_corpus.json`.
- [x] Метрики через `scripts/eval_pipeline.py`: sweep `sim_threshold` + end-to-end
      (анти-галлюцинация, recall, точность ключевого факта, источник, латентность).
- [x] Профиль RAM: Ollama ~2 ГБ, Postgres ~60 МБ (лимит 512) → ~2.1 ГБ из 16;
      латентность p50=0.5s, p95=1.2s (3B-модель на M5). `MAX_CONCURRENT_LLM=1` оставлен.
- [x] Юнит-тесты: чанкование, гибридный скор, grounding (`tests/`, 11 passed).
- **Критерий приёмки ✅**: out-of-scope → заглушка **100%** (цель ≥90%);
  p95 латентности **1.2s** зафиксирован; recall grounded 90%.
- **Нюанс калибровки:** с одним чанком в БД retrieval-порог неразличим —
  анти-галлюцинацию обеспечивает grounding-слой генерации (многоуровневая защита).
  Полноценная калибровка `sim_threshold` требует более богатого корпуса (post-MVP).
